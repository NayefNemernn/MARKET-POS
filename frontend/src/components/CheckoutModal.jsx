...
export default function CheckoutModal({ cart, total, close }) {
  ...
  useEffect(() => {
    ...
    setChange(Math.max(received - total, 0));
  }, [amount, total]);
  ...
  const completeSale = async () => {
    ...
    try {
      ...
      // PAY LATER — always needs internet (creates customer record on server)
      if (method === "later") {
        if (!customerName.trim()) { alert(t.enterCustomerName); return; }

        if (!navigator.onLine) {
          toast.error("Pay Later requires an internet connection");
          return;
        }
      ...
      if (navigator.onLine) {
        ...
        // 📴 Offline — save locally, show receipt from cart data
  ...
  if (receipt) {
    return (
      <div className="fixed inset-0 bg-black/80 backdrop-blur flex items-center justify-center z-50">
        <div className="
          bg-gray-100 dark:bg-[#141414] rounded-3xl p-6 w-[360px]
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur flex items-center justify-center z-50">
      <div className="
        w-[420px] bg-gray-100 dark:bg-[#141414] rounded-3xl p-6
        shadow-[10px_10px_25px_#d1d5db,-10px_-10px_25px_#ffffff]
        dark:shadow-[10px_10px_25px_#050505,-10px_-10px_25px_#1f1f1f]
      ">
        ...
        {/* Total */}
        <div className="bg-blue-600 text-white rounded-2xl p-6 text-center mb-6 shadow-[0_0_20px_rgba(59,130,246,0.6)]">
          <p className="text-blue-200 text-sm">{t.totalAmount}</p>
        ...
        <div className="flex gap-3 mb-4">
          ...
          <button
            ...
            className={`flex-1 py-3 rounded-xl transition
              ${method === "later" ? "bg-purple-600 text-white" : "bg-gray-200 dark:bg-[#1c1c1c]"}
              ${!navigator.onLine ? "opacity-40 cursor-not-allowed" : ""}
            `}
          >
            {t.payLater}
          </button>
        ...
        {method === "cash" && (
          <>
            ...
            <div className="grid grid-cols-4 gap-2 mb-4">
              {[20, 50, 100].map(v => (
                <button key={v} onClick={() => setAmount(v)} className="bg-gray-200 dark:bg-[#1c1c1c] py-2 rounded-lg">${v}</button>
              ))}
              <button onClick={() => setAmount(total)} className="bg-gray-200 dark:bg-[#1c1c1c] py-2 rounded-lg">{t.exact}</button>
            </div>
          </>
        )}
