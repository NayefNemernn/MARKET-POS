...
router.patch("/online-settings", isAdmin, async (req, res) => {
  try {
    const { isOnlineStoreActive, deliveryFee, minimumOrder, telegramBotToken, deliveryDrivers } = req.body;
    const store = await Store.findByIdAndUpdate(
      req.storeId,
      { $set: { isOnlineStoreActive, deliveryFee, minimumOrder, telegramBotToken, deliveryDrivers } },
      { new: true }
    );
